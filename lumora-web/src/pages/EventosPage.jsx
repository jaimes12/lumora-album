...
export default function EventosPage() {
  ...
  const filtered = (dateFrom || dateTo)
    ? bySearch.filter(e => {
        const iso = dateField === 'event' ? e.dateISO : e.createdAtISO
  ...
  useEffect(() => { reset() }, [filter, search, dateFrom, dateTo, dateField, reset])
    ...
    <div className={styles.page}>
      ...
      <div className={styles.toolbarRow}>
        ...
        <div className={styles.toolbarRight}>
          ...
          {/* Filtro fecha */}
          <div className={styles.dateFilterWrap} ref={dateFilterRef}>
            <button
              className={`${styles.dateFilterBtn} ${hasDateFilter ? styles.dateFilterBtnActive : ''}`}
            ...
            {showDateFilter && (
              <div className={styles.dateFilterDropdown}>
                <div className={styles.dateFieldToggle}>
                  <button
                    className={`${styles.dateFieldBtn} ${dateField === 'event' ? styles.dateFieldBtnActive : ''}`}
                    onClick={() => setDateField('event')}
                  >Fecha del evento</button>
                  <button
                    className={`${styles.dateFieldBtn} ${dateField === 'created' ? styles.dateFieldBtnActive : ''}`}
                    onClick={() => setDateField('created')}
                  >Fecha de creación</button>
                </div>
                ...
                {hasDateFilter && (
                  <button className={styles.dateFilterClearBtn} onClick={() => { setDateFrom(''); setDateTo('') }}>
                    Limpiar filtro
                  </button>
                )}
              </div>
        ...
        <div className={styles.empty}>
          <p>
            {q
              ...
              : hasDateFilter
                ? `Sin eventos en el rango de fechas seleccionado`
                : `Sin eventos${filter !== 'todos' ? ` con estado "${ESTADO_META[filter]?.label}"` : ''}`
            }
          </p>
          {!q && !hasDateFilter && <button className={styles.btnPrimary} onClick={() => setShowCreate(true)}>+ Crear primer evento</button>}
        </div>
      ) : (
        <>
          <div className={styles.grid}>
