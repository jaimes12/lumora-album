...
const NAV_KEYS_BASE = [
  ...
  // index 3 — replaced dynamically by getNavItems() with eventos or viajes
  null,
  {
  ...
  {
    to: '/app/finanzas', key: 'finanzas',
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/><line x1="7" y1="15" x2="7.01" y2="15" strokeWidth="2.5" strokeLinecap="round"/><line x1="11" y1="15" x2="13" y2="15"/></svg>,
  },
  {
    to: '/app/cuentas', key: 'cuentas', adminOnly: true,
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="3" y1="15" x2="21" y2="15"/><line x1="9" y1="9" x2="9" y2="21"/><line x1="15" y1="9" x2="15" y2="21"/></svg>,
  },
  {
    to: '/app/notas', key: 'notas', plansAllowed: ['agencia'],
...
const VIAJES_NAV_ITEM = {
  to: '/app/viajes', key: 'viajes',
...
const MARKET_NAV_ITEM = {
  to: '/app/market', key: 'market',
  icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l1.5-5h15L21 9"/><path d="M3 9v10a1 1 0 0 0 1 1h16a1 1 0 0 0 1-1V9"/><path d="M3 9h18"/><path d="M9 21v-6h6v6"/></svg>,
}

const EVENTOS_NAV_ITEM = {
        ...
        <div className={styles.sidebarInner}>
          ...
          <nav className={styles.nav}>
            {navItems.map(item => {
              ...
              const planLocked = !locked && item.plansAllowed && !item.plansAllowed.includes(user?.plan)
              if (locked || planLocked) {
                return (
                  <div key={item.key} className={`${styles.navItem} ${styles.navItemLocked}`}
                    onClick={() => { setSidebarOpen(false); navigate('/app/paquetes') }}
                    title={planLocked ? `Disponible en plan ${item.plansAllowed?.join('/')}` : 'Elige un plan para acceder'}
                  >
